...
ilr_comp <- pca$center + scores%*%pca$rotation
clr_comp <- ilr_comp%*%basis_matrix
norm_constant <- sum(exp(clr_comp))
...
```{r MCEM with original data, eval=FALSE}
...
testrun2 <- fit_compositional_pca_db(count_data, max_iter = 50, r = 10, lambda = 1, eps = 0.01)
biplot(testrun2$pca)
```

Original error when running with version 1: 
Error in fit_compositional_pca(count_data, max_iter = 50, r = 10, lambda = 1,  : 
  Infinite or NaN values found in weights at iteration 1 for observation 2119

testrun2 <- fit_compositional_pca_db(count_data, max_iter = 50, r = 10, lambda = 1, eps = 0.01)
[1] "non finite weights:"
 [1]   0 NaN NaN   0 NaN   0   0 NaN   0 NaN
Error in fit_compositional_pca_db(count_data, max_iter = 50, r = 10, lambda = 1,  : 
  Infinite or NaN values found in weights at iteration 1 for observation 2119

That obviously creates some problems, that we be summarised under error messages.
...
testrun3 <- fit_compositional_pca_ilr(x_data_reduced, max_iter = 50, r = 10, lambda = 1, eps = 0.01)
biplot(testrun3$pca)
```
