...
```{r adjusted conditional scores density, eval=TRUE}
# objective function
...
# Numerische Stabilisierung durch Abziehen des Maximums
...
gradient_cslc_adjusted <- function(scores, x_data_i, pca){
...
! invalid arguments
Backtrace:
 1. global fit_compositional_pca_db(...)
 2. base::sapply(...)
...
```{r test simulation 2, eval=FALSE}
testrun3 <- fit_compositional_pca(x_data, max_iter = 50, r = 10, lambda = 1, eps = 0.01)
biplot(testrun3$pca)
```
...
for(i in 1:length(x_data)){
        ...
        optim_result <- optim(rep(0, length = length(pca$sdev)), conditional_scores_log_clr_composition, gr = gradient_cslc,
                              x_data_i = x_data[[i]], pca = pca,
                              control = list(fnscale = -1), method = "BFGS")
        scores_list[[i]] <- as.vector(optim_result$par)
}
```
