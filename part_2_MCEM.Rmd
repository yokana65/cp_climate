...
# run with scaling
testrun_scaling <- fit_compositional_pca_ilr_sc(count_data, max_iter = 30, r = 15, lambda = 1, eps = 0.01)
testrun_sc001 <- fit_compositional_pca_ilr_sc(count_data, max_iter = 30, r = 15, lambda = 1, eps = 0.01, sc_factor = 0.001)
testrun_sc001 <- fit_compositional_pca_ilr_sc(count_data, max_iter = 30, r = 15, lambda = 0.8, eps = 0.01, sc_factor = 0.005)


plot_pca_rotation(testrun_scaling$pca$rotation)
plot(testrun_scaling$pca$sdev)
```
...
```{r adjusted mcem, eval=TRUE}

fit_compositional_pca_ilr_sc_db <- function(x_data,
                                         max_iter = 50,
                                         r = 10,
                                         lambda = 1,
                                         eps = 0.01,
...
gradient_cslc_ilr_sc <- function(scores,
  ...
  rotation_matrix <- pca$rotation%*%basis_matrix*(-1)
  # equivalent to
  # rotation_matrix_2 <- ilr2clr(pca$rotation)

  grad <- sapply(seq_along(scores), function(k) {
    e_k <- rotation_matrix[, k]
    term1 <- sum(x_data_i * e_k)
    term2 <- m_i * sum(composition * e_k)
...
conditional_scores_log_ilr_sc <- function(scores, x_data_i, pca, basis_matrix, sc_factor) {
...
gradient_cslc_ilr_sc <- function(scores, x_data_i, pca, basis_matrix, sc_factor) {
  ...
  if(max_iter > 0){
    for(k in 1:max_iter){
      ...
      for(i in 1:length(x_data)){
        ...
        # check value of the conditional distribution
        conditional_scores_list[[i]] <- conditional_scores_log_ilr_sc(scores, x_data[[i]], pca, basis_matrix, sc_factor) 

        log_weights <- apply(proposal_scores[[i]], 2, function(scores){
          conditional_scores_log_ilr_sc(scores, x_data[[i]], pca, basis_matrix, sc_factor) -
            sum(dnorm(scores, mean = scores_median, sd = lambda*pca$sdev, log = TRUE))
        })
        # increase numerical stability
        if (sum_exp) {
...
part_2_MCEM.Rmd Stell dir vor du bist mein Statistikprofessor und wir arbeiten schon viele Jahre vertrauensvoll und professionell zusammen. Ich bin Student an deinem Lehrstuhl und möchte jetzt meine Masterarbeit in Statistik schreiben. Du bist besonders gut in der Lage komplexe Sachverhalte einfach zu erklären, weswegen du von Studenten und Kollegen sehr geschätzt wirst. Außerdem bist du eine sehr unterstützende Persönlichkeit. Dein Kerngebiet ist die Analyse von Kompositionsdaten. Ich möchte meine Masterarbeit gerne in Kooperation mit dem geowissenschaftlichen Institut der Uni Potsdam schreiben. Diese arbeiten mit einem Datensatz von einer Sedimentbohrung im Golf von Aden, mit dem die Mineralienzusammensetzung des Sedimentes bestimmt werden kann. Mit Hilfe eines Altersmodells lassen sich damit Rückschlüsse auf die letzten 500.000 Jahre von Klimaprozessen schließen. Es wurde bereits vielfältig zum Datensatz von der geowissenschaftlichen Arbeitsgruppe publiziert. Allerdings fehlt bisher eine statistisch fundierte Analyse des Datensatzes als Komposition. Dies möchte ich mit Hilfe einer umfassenden Analyse im framework der compositional data analysis umsetzen. Ich habe den MCEM algorithmus über die ilr-Koordinaten implementiert. Nun möchte ich den Algorithmus mit Hilfe von simulierten Daten validieren. Dabei habe ich aber noch ein Verständnisproblem. Meine beobachteten Daten sind als zurücktransformierte (i.e. clr-inverse-Transformierung) Ergebnisse eines latenten Prozesses modelliert, der als multivariate Normalverteilung strukturiert ist. Ich kann den Mittelwertvektor festsetzen und die Eigenwerte der Kovarianzmatrix festlegen, um einen solchen Prozess zu simulieren. Aber wie kann ich den Einfluss der Kompositionsteile auf die Hauptkomponenten bestimmen? Z.B. wenn ich eine Komposition aus fünf Teilen habe und simulieren möchte, dass der Kontrast zwischen dem zweiten und dem dritten Teil die erste Hauptkomponente bestimmt, während die zweite Hauptkomponente durch den dritten Teil bestimmt ist. Kannst du mir Schritt für Schritt erklären, wie ich das umsetzen kann und wie dafür der Programmiercode aussehen würde?

1. construct latent compositions

```{r simulate latent compositions, eval=TRUE}
library(mvtnorm)
# Define dimension
D <- 5
...
2. Simulate count compositions with fixed m_i

```{r simulate count composition, eval=TRUE}
...
```{r test simulation, eval=TRUE}
# first of all test with standard pca
# change list into matrix
x_matrix <- do.call(rbind, x_data)
clr_data <- clr(x_matrix)
pca_1 <- prcomp(clr_data)
plot_pca_rotation(pca_1$rotation)
plot(pca_1$sdev)
plot(pca_1$center)

```

Interestingly enough, PCA is not really good in identifying the correct principal components. 

What do we get from our algorithm (n=100, m_1 = 2000)?

```{r test simulation mcem, eval=TRUE}
pca_mcem <- fit_compositional_pca_ilr_sc_db(x_data, sc_factor = 1) 
plot_pca_rotation(pca_mcem$pca$rotation)
sim_data <- tar_read(simulation_composition_1)
sim_data <- sim_data$x_data
pca_old <- fit_compositional_pca_vs1_0(sim_data) 
...
for (i in seq_along(x_data)) {
  result <-
    optim(rep(0, length = length(pca$sdev)),
          ...
          control = list(fnscale = -1),
          method = "BFGS")
  scores_median_list[[i]] <- result$par
}
scores_median_list
scores_0 <- rep(0, length = length(pca$sdev))
scores <- scores_median_list[[1]]
scores_1 <- ilr(x_data[[1]]) 
scores_clr <- clr(x_data[[1]]) 

ilr_comp <- pca$center + pca$rotation %*% scores_0
clr_comp <- ilr2clr(ilr_comp)
```

Check the gradient:

```{r check gradient, eval=TRUE}
...
pca_mcem <-fit_compositional_pca_ilr_sc(x_data, max_iter = 40, r = 10, lambda = 1, eps = 0.01)
plot_pca_rotation(pca_mcem$pca$rotation)
...
Given our objective function, what do we actually calulate for the conditional scores?

```{r likelihood_estimation, eval=FALSE}
# Be aware that a standard vektor in R is a column vektor
# i.e. the row vektors are transposed vektors
x <- c(1,2,3)
dim(as.matrix(x)) 


x_data <- tar_read(data_kl15_comp)
count_data <- apply(x_data, 1, function(x) x, simplify = FALSE)
x_data_i <- count_data[[1]]

x_ilr <- ilr(x_data)
pca <- prcomp(x_ilr, scale = FALSE)

pca_1 <- tar_read(pca_count_ilr_std)
pca <- pca_1$pca
sc_factor <- 1
scores_1 <- ilr(x_data_i)
scores_2 <- pca$center 
conditional_scores_log_ilr_sc <- function(scores,
                                          x_data_i,
                                          pca,
                                          ...
                                          sc_factor) {
  scaling_factor <- sc_factor
  ilr_comp <- pca$center + pca$rotation %*% t(scores)
  # clr_comp <- t(ilr_comp) %*% basis_matrix
  clr_comp <- ilr2clr(ilr_comp)
  norm_constant <- sum(exp(clr_comp))
...
# alternative pca input



# Calculate max cond scores
conditional_scores_log_ilr_sc(scores_1,
                                          x_data_i,
                                          pca,
                                          basis_matrix,
                                          sc_factor) 
...
The conditional scores is maximized at $z_i = ilr(x_i)$ and pca being the standard pca onto ilr(x).

Does this hold true, when we apply the scaling factor?

```{r test conditional, eval=FALSE}
  ...
  pca_in <- list()
  eigen_decomp <- eigen(Sigma)
  pca_in$rotation <- eigen_decomp$vectors
  pca_in$sdev <- eigen_decomp$values
  pca_in$center <- nu
...
test <- fit_compositional_pca_ilr_sc(count_data, sc_factor = 1)
```

# Final Test

Get results and make sense of them:
...
Be aware that we are estimating ilr-coefficients

```{r results 1, eval=TRUE}
pca_1 <- tar_read(pca_count_ilr_std)
# Get rownames
data_comp <- tar_read(data_kl15_comp)
colnames(data_comp)
pca_results <- pca_1$pca
rotation_ilr <- pca_results$rotation
rotation_clr <- t(rotation_ilr%*%basis_matrix)
rownames(rotation_clr) <- colnames(data_comp)
plot_pca_rotation(rotation_clr)
...
```{r results scree plot 1, eval=TRUE}
plot(pca_results$sdev)
```


```{r results center plot 1, eval=TRUE}
plot(pca_results$center)
```


```{r results evolution plot 1, eval=TRUE}
pca_1$list_center
```