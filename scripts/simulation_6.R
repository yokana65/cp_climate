...
complex_setting_autoc <- function(data,
  ...
  generate_ar1 <- function(n, rho, sigma) {
      eps <- rnorm(n, 0, sigma)
      x <- numeric(n)
      x[1] <- eps[1]
      for(t in 2:n) {
  ...
  # Generiere autokorrelierte Koeffizienten
  coef1 <- generate_ar1(n_observations, rho, sqrt(eigenvalues[1]))
  coef2 <- generate_ar1(n_observations, rho, sqrt(eigenvalues[2]))
  coef3 <- generate_ar1(n_observations, rho, sqrt(eigenvalues[3]))
  coef4 <- generate_ar1(n_observations, rho, sqrt(eigenvalues[4]))
...
complex_setting_auto_sin <- function(data,
                          eigenvalues = c(0.43, 0.26, 0.12, 0.08),
                          mean = c(-1.21, -1.76, 1.70, -0.6,
                                 1.04, -3.62, -1.26, 1.04, -0.83,
                                 0.50, -4.0, -0.49, 1.51),
