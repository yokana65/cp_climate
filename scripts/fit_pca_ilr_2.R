...
# change csc and gradient
fit_pca_vs_7 <- function(x_data,
                         max_iter = 50,
                         r = 10,
                         lambda = 1,
                         eps = 0.01,
  ...
  if (max_iter > 0) {
    for (k in 1L:max_iter) {
      for (i in seq_along(x_data)){
        optim_result <- optim(rep(0L, length = length(pca$sdev)),
                              conditional_scores_log_ilr_vs2,
                              gr = gradient_cslc_vs3,
                              x_data_i = x_data[[i]],
                              pca = pca,
                              basis_matrix = V,
                              sc_factor = sc_factor,
...
fit_pca_ilr_vs_8 <- function(x_data,
                             max_iter = 50,
                             r = 10,
                             lambda = 1,
                             eps = 0.01,
  ...
  if (max_iter > 0) {
    for (k in 1L:max_iter) {
      ...
      if (max(critical_value_1, critical_value_2) < eps) {
        ...
        pca$scores <- if (scores) {
          clrInv(pca$scores_clr)
          }
        rownames(pca$rotation) <- names(x_data[[1L]])
        cn <- paste0("Comp.", seq_len(ncol(pca$rotation)))
        colnames(pca$rotation) <- cn
