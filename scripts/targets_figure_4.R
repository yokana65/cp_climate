...
ess_data <- data.frame(
  ESS = c(
    simulation_results_list[[1]][[1]]$ESS,
    simulation_results_list[[2]][[1]]$ESS,
    simulation_results_list[[3]][[1]]$ESS,
    simulation_results_list[[4]][[1]]$ESS
  ),
...
plot1 <- ggplot(ess_data, aes(x = ESS, fill = size)) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of sample sizes for important sampling",
