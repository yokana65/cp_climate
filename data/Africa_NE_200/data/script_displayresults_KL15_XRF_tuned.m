...
f(1) = figure('Position',[0 00 800 1200],...
    'Color',[1 1 1]);
...
%b = abs(min(data(:,varselectnum(1)))) +  abs(max(min(data(:,varselectnum(1)))))
...
%for i = 1:7
 %   
  % ia= i*2
   % if MISp(ia) < agemodelmax 
    %     rectangle('Position',[MISp(ia) min(data(:,varselectnum(1))) MISp(ia)+2 b],'FaceColor',[0.9 0.9 0.9],'LineStyle','none')
    %end
%end
...
if agerefs == 1 
        for i = 1:20
         if MIS(i) < agemodelmax 
               ...
               %old try tu jsut get triangles at the top of the plot °_°
               %YL = get(gca, 'YLim');
