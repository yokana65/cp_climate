...
# Implementation in deiner Schleifenstruktur
for(j in 1:4) {
  simulation_data <- simulation_data_list_2[[j]]
  ...
  for(i in 1:length(simulation_data)) {
    ...
    stats <- summary_stats(x_data)
    print(paste("Scenario", j, "Simulation", i))
    print(stats)
    
    # Optional: Zusätzliche Visualisierung
    boxplot(x_data, main=paste("Scenario", j, "Simulation", i))
  }
}
```
...
# descriptives for each simulation
for(j in 1:4) {
  simulation_data <- simulation_data_list_14[[j]]
  for(i in 1:length(simulation_data)) {
    sim <- simulation_data[[i]]
