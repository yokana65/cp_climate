...
$i=1,\ldots,100$.  
The simulated dataset is then used to evaluate the performance of the MCEM algorithm. That is done by evaluating the MCEM algorithm's capability to identify the 
...
covariance structure derived from the principal component analysis is compared with the result of both a classical and a robust PCA.  
It is important to note that the MCEM algorithm operates directly on the simplex, while both classical and robust PCA are performed on the transformed coordinates 
of the observations.  
The classical PCA is conducted with the `prcomp` base function in R using a singular value decomposition of the clr-transformed coordinates. This is equivalent to an 
...
$z_{i 1} \stackrel{i . i . d .}{\sim} Z_{1}$ and $z_{i 2} \stackrel{i . i . d .}{\sim} Z_{2}$. The factors associated with the two principal components are normally distributed 
...
method shows on average, a clear improvement compared to both classical and robust PCA. Furthermore it is much better in determining the true structure of the mean vectors 
...
Interestingly, the opposite effect is observed when comparing classical PCA with its robust variant. The robust computation of PCA offers some advantages at moderate sample 
...
plot_pca_rotation_sim <- function(rotation_list1, rotation_list2, rotation_list3, scale = 1, main = "PCA Comparison") {
    ...
    df_all$method <- factor(df_all$method, 
                           levels = c("MCEM", "classic PCA", "robust PCA", "oracle PC"))
...
classic_rotation_list <- list(n_simulations)
...
classic_rotation_list <- lapply(seq_along(pca_clr_std_2[[2]]), function(j) {
...
the classical Method underestimates the variation in the second principal components and the robust method overestimates it.

```{r mean directions, eval=TRUE}
rotation_list1 <- mcem_rotation_list
rotation_list2 <- classic_rotation_list
...
scale <- 1
    ...
    df_all$method <- factor(df_all$method, 
                           levels = c("MCEM", "classic PCA", "robust PCA"))
...
for(i in seq_along(simulation_results_list_2[[1]])) {
  ...
  # Classic PCA eigenvalues
  classic_eig <-  pca_clr_std_2[[1]][[i]]$sdev^2
  classic_eig <- normalize(classic_eig)
  ...
  scree_df <- rbind(scree_df, 
                    data.frame(eigenvalue = c(mcem_eig, classic_eig[1:4], robust_eig),
                             component = rep(1:length(mcem_eig), 3),
                             method = rep(c("MCEM", "classic PCA", "robust PCA"), 
...
And the covariance structure for the oracle, the MCEM estimate and the classic and robust PCA estimate.
...
sigma_diff_df_7 <- data.frame(
  ...
  method = factor(rep(c("MCEM", "classic"), each = 4 * n_simulations)),
  size = factor(rep(rep(c("0.0001", "0.001", "0.01", "0.05"), 
...
diff_mean_rob_clr_12 <- lapply(1:4, function(x) list(n_simulations))

for(j in 1:4) {
  for(i in 1:n_simulations) {
    diff_mean_12[[j]][[i]] <- sqrt(sum((true_v_12 - t(basis_matrix) %*% simulation_results_list_12[[j]][[i]]$pca$center)^2))
    diff_mean_rob_clr_12[[j]][[i]] <- sqrt(sum((true_v_12 - pca_clr_rob_12[[j]][[i]]$center)^2))
  }
}

diff_mean = diff_mean_12
diff_mean_rob_clr = diff_mean_rob_clr_12
