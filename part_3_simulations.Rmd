...
for(j in 1:4) {
  ...
  for(i in 1:length(simulation_data)) {
    ...
    pca_clr_std_17[[j]][[i]] <- prcomp(clr_transform(x_data, replace_zeros = "fraction"))
    x_data_repl <- x_data[x_data == 0] <- 0.65 * 0.5/1
    pca_clr_rob_17[[j]][[i]] <- pcaCoDa(x_data, method = "robust")
  }
}
...
## Simulation Setting 18: final balanced plot + zero replacement + princomp
...
for(j in 1:4) {
  ...
  for(i in 1:length(simulation_data)) {
    ...
    pca_clr_std_18[[j]][[i]] <- princomp(clr_transform(x_data, replace_zeros = "fraction"))
    x_data_repl <- x_data[x_data == 0] <- 0.65 * 0.5/1
    pca_clr_rob_18[[j]][[i]] <- pcaCoDa(x_data, method = "robust")
  }
}
...
for(j in 1:4) {
  for(i in 1:n_simulations) {
    ...
    sigma_hat_clr <- with(pca_clr_std_18[[j]][[i]],
      loadings %*% diag(sdev^2) %*% t(loadings))
...
for(j in 1:4) {
  ...
  for(i in 1:length(simulation_data)) {
    ...
    pca_clr_std_19[[j]][[i]] <- prcomp(clr_transform(x_data, replace_zeros = "fraction"))
    x_data_repl <- x_data[x_data == 0] <- 0.65 * 0.5/1
...
for(j in 1:4) {
  ...
  for(i in 1:length(simulation_data)) {
    ...
    pca_clr_std_20[[j]][[i]] <- prcomp(clr_transform(x_data, replace_zeros = "fraction"))
    x_data_repl <- x_data[x_data == 0] <- 0.65 * 0.5/1
...
## Simulation Setting 21: complex with n=2119
...
for(j in 1:4) {
  ...
  for(i in 1:length(simulation_data)) {
    ...
    pca_clr_std_21[[j]][[i]] <- prcomp(clr_transform(x_data, replace_zeros = "fraction"))
    x_data_repl <- x_data[x_data == 0] <- 0.65 * 0.5/1
...
diff_df_21$size <- factor(diff_df_21$size, 
                        levels = c("cts per sec", "cts per dsec", "cts per csec", "cts per msec"))

plot1 <- ggplot(diff_df_21, aes(x = size, y = differences, fill = method)) +
  geom_boxplot() +
  theme_grey() +
  scale_fill_manual(values = set_1) + 
  labs(y = "dist mean",
...
```{r visualize covariances 21, eval=TRUE}
sigma_diff_df_21 <- data.frame(
  differences = c(
    unlist(sigma_distances_21[[1]]), unlist(sigma_distances_21[[2]]), 
    unlist(sigma_distances_21[[3]]), unlist(sigma_distances_21[[4]]),
    unlist(sigma_distances_std_clr_21[[1]]), unlist(sigma_distances_std_clr_21[[2]]),
    unlist(sigma_distances_std_clr_21[[3]]), unlist(sigma_distances_std_clr_21[[4]]),
    unlist(sigma_distances_rob_clr_21[[1]]), unlist(sigma_distances_rob_clr_21[[2]]),
    unlist(sigma_distances_rob_clr_21[[3]]), unlist(sigma_distances_rob_clr_21[[4]])
  ),
  method = factor(rep(c("MCEM", "standard","robust"), each = 4 * n_simulations)),
  size = factor(rep(rep(c("cts per sec", "cts per dsec", "cts per csec", "cts per msec"), 
                         each = n_simulations), 2))
...
plot2 <- ggplot(sigma_diff_df_21, aes(x = size, y = differences, fill = method)) +
  geom_boxplot() +
  theme_grey() +
  scale_fill_manual(values = set_1) + 
  labs(y = "dist covariance",
...
## Simulation Setting 22: complex with n=2119 + autocorrelation
...
for(j in 1:4) {
  ...
  for(i in 1:length(simulation_data)) {
    ...
    pca_clr_std_22[[j]][[i]] <- prcomp(clr_transform(x_data, replace_zeros = "fraction"))
    x_data_repl <- x_data[x_data == 0] <- 0.65 * 0.5/1
...
diff_df_22 <- data.frame(
  differences = c(
    ...
    unlist(diff_mean_22[[3]]), unlist(diff_mean_22[[4]]),
    unlist(diff_mean_std_clr_22[[1]]), unlist(diff_mean_std_clr_22[[2]]),
    unlist(diff_mean_std_clr_22[[3]]), unlist(diff_mean_std_clr_22[[4]])
...
plot1 <- ggplot(diff_df_22, aes(x = size, y = differences, fill = method)) +
  geom_boxplot() +
  theme_grey() +
  scale_fill_manual(values = set_1) + 
  labs(y = "dist mean",
...
```{r visualize covariances 22, eval=TRUE}
sigma_diff_df_22 <- data.frame(
  differences = c(
    unlist(sigma_distances_22[[1]]), unlist(sigma_distances_22[[2]]), 
    unlist(sigma_distances_22[[3]]), unlist(sigma_distances_22[[4]]),
    unlist(sigma_distances_std_clr_22[[1]]), unlist(sigma_distances_std_clr_22[[2]]),
    unlist(sigma_distances_std_clr_22[[3]]), unlist(sigma_distances_std_clr_22[[4]]),
    unlist(sigma_distances_rob_clr_22[[1]]), unlist(sigma_distances_rob_clr_22[[2]]),
    unlist(sigma_distances_rob_clr_22[[3]]), unlist(sigma_distances_rob_clr_22[[4]])
  ),
  method = factor(rep(c("MCEM", "standard","robust"), each = 4 * n_simulations)),
  size = factor(rep(rep(c("cts per sec", "cts per dsec", "cts per csec", "cts per msec"), 
                         each = n_simulations), 2))
...
plot2 <- ggplot(sigma_diff_df_22, aes(x = size, y = differences, fill = method)) +
  geom_boxplot() +
  theme_grey() +
  scale_fill_manual(values = set_1) + 
  labs(y = "dist covariance",
