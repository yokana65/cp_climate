...
for(j in 1:4) {
  for(i in 1:n_simulations) {
    diff_oracle_mean_14[[j]][[i]] <- sqrt(sum((oracle_mean[[j]][[i]] - t(basis_matrix) %*% simulation_results_list_14[[j]][[i]]$pca$center)^2))
    diff_oracle_mean_std_clr_14[[j]][[i]] <- sqrt(sum((oracle_mean[[j]][[i]] - pca_clr_std_14[[j]][[i]]$center)^2))
    diff_oracle_mean_rob_clr_14[[j]][[i]] <- sqrt(sum((oracle_mean[[j]][[i]] - pca_clr_rob_14[[j]][[i]]$center)^2))
  }
}
...
for(j in 1:4) {
  for(i in 1:n_simulations) {
    # MCEM method
    comp_pca <- simulation_results_list_14[[j]][[i]]$pca
    comp <- predict_latent_compositions_clr(comp_pca, basis_matrix = basis_matrix, sc_factor = 1)
    list <- comp$compositions
    if(length(list) != n_simulations) {
      stop("Error: The list is not correct.")
    }
    df <- do.call(rbind, list)
    est_covar <- cov(df)
    sigma_distances_14[[j]][[i]] <- norm(oracle_covar - est_covar, type = "F")
