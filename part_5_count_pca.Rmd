...
# pca_count <- tar_read(pca_count_ilr_vs1_1perc_ep04)
# pca_count <- tar_read(pca_count_ilr_vs1_1perc_ep03)
pca <- pca_count$pca
...
See Filzmoser et al. 2019, p.131ff

```{r robust PCA, eval=TRUE, fig.width=8, fig.height=8}
x_count <- tar_read(data_kl15)
x_clr <- tar_read(data_kl15_comp_clr)
x_count_sel <- x_count[4:ncol(x_count)-1]
colnames(x_count_sel) <- gsub("_cts", "", colnames(x_count_sel))
x_acomp <- acomp(x_count_sel)
colnames(x_acomp) <- gsub("_cts", "", colnames(x_acomp))
# pca_classic <- princomp(x_acomp)
pca_rob <- pcaCoDa(x_count_sel)

# with centered data
x_cen <- x_acomp -mean(x_acomp )
pca_rob_2 <- pcaCoDa(x_cen)
...
norm_eigv <- pca_rob$eigenvalues / sum(pca_rob$eigenvalues)

```

```{r screeplot 2, eval=TRUE}
plot(pca_rob)
...
<!-- We can calculate the percentage of explained variance by the principal components with the **variation matrix**: -->
The first two components explain `r sum(pca_rob$eigenvalues[1:2])/sum(pca_rob$eigenvalues) * 100` % of the total variance.  
The first four components explain `r sum(pca_rob$eigenvalues[1:4])/sum(pca_rob$eigenvalues) * 100` % of the total variance.  
The first component explains `r pca_rob$eigenvalues[1]/sum(pca_rob$eigenvalues) * 100` % of the total variance.  
The second component explains `r pca_rob$eigenvalues[2]/sum(pca_rob$eigenvalues) * 100` % of the total variance.  
The third component explains `r pca_rob$eigenvalues[3]/sum(pca_rob$eigenvalues) * 100` % of the total variance.  
The fourth component explains `r pca_rob$eigenvalues[4]/sum(pca_rob$eigenvalues) * 100` % of the total variance.  

```{r biplots, eval=TRUE, fig.width=8, fig.height=8}
plot_pca_rotations(pca_rob$loadings, components = c(1,3), main = "PC1 vs. PC3", fixed=TRUE)
plot_pca_rotations(pca_rob$loadings, components = c(1,4), main = "PC1 vs. PC4", fixed=FALSE)
plot_pca_rotations(pca_rob$loadings, components = c(1,5), main = "PC1 vs. PC5", fixed=TRUE)
plot_pca_rotations(pca_rob$loadings, components = c(2,3), main = "PC2 vs. PC3", fixed=TRUE)
plot_pca_rotations(pca_rob$loadings, components = c(2,4), main = "PC2 vs. PC4", fixed=FALSE)
plot_pca_rotations(pca_rob$loadings, components = c(2,5), main = "PC2 vs. PC5", fixed=TRUE)
plot_pca_rotations(pca_rob$loadings, components = c(3,4), main = "PC3 vs. PC4", fixed=FALSE)
plot_pca_rotations(pca_rob$loadings, components = c(3,5), main = "PC3 vs. PC5", fixed=TRUE)
plot_pca_rotations(pca_rob$loadings, components = c(4,5), main = "PC4 vs. PC5", fixed=TRUE)
```


```{r principal components rob, eval=TRUE, fig.width=12, fig.height=9}
center_clr <- mean(x_acomp)
pca_rotation_clr <- pca_rob$loadings
pca_rotation_splx <- clrInv(pca_rotation_clr)

# Create data frame for plotting
mean_comp_1 <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = pca_rotation_clr[,1] * sqrt(pca_rob$eigenvalues[1]),
  pc1_minus = - pca_rotation_clr[,1] * sqrt(pca_rob$eigenvalues[1])
)
...
mean_comp_2 <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = pca_rotation_clr[,2] * sqrt(pca_rob$eigenvalues[2]),
  pc1_minus =  - pca_rotation_clr[,2] * sqrt(pca_rob$eigenvalues[2])
)

mean_comp_3 <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_clr[,3] * sqrt(pca_rob$eigenvalues[3]),
  pc1_minus = - pca_rotation_clr[,3] * sqrt(pca_rob$eigenvalues[3])
)

mean_comp_4 <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_clr[,4] * sqrt(pca_rob$eigenvalues[4]),
  pc1_minus = - pca_rotation_clr[,4] * sqrt(pca_rob$eigenvalues[4])
)

mean_comp_5 <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_clr[,5] * sqrt(pca_rob$eigenvalues[5]),
  pc1_minus = - pca_rotation_clr[,5] * sqrt(pca_rob$eigenvalues[5])
)

mean_comp_6 <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_clr[,6] * sqrt(pca_rob$eigenvalues[6]),
  pc1_minus = - pca_rotation_clr[,6] * sqrt(pca_rob$eigenvalues[6])
)

mean_comp_1_splx <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = pca_rotation_splx[,1] * sqrt(pca_rob$eigenvalues[1]),
  pc1_minus = - pca_rotation_splx[,1] * sqrt(pca_rob$eigenvalues[1])
)

mean_comp_2_splx <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = pca_rotation_splx[,2] * sqrt(pca_rob$eigenvalues[2]),
  pc1_minus =  - pca_rotation_splx[,2] * sqrt(pca_rob$eigenvalues[2])
)

mean_comp_3_splx <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_splx[,3] * sqrt(pca_rob$eigenvalues[3]),
  pc1_minus = - pca_rotation_splx[,3] * sqrt(pca_rob$eigenvalues[3])
)

mean_comp_4_splx <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_splx[,4] * sqrt(pca_rob$eigenvalues[4]),
  pc1_minus = - pca_rotation_splx[,4] * sqrt(pca_rob$eigenvalues[4])
)

mean_comp_5_splx <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_splx[,5] * sqrt(pca_rob$eigenvalues[5]),
  pc1_minus = - pca_rotation_splx[,5] * sqrt(pca_rob$eigenvalues[5])
)

mean_comp_6_splx <- data.frame(
  component = 1:13,
  mean_value = 0,
  pc1_plus = + pca_rotation_splx[,6] * sqrt(pca_rob$eigenvalues[6]),
  pc1_minus = - pca_rotation_splx[,6] * sqrt(pca_rob$eigenvalues[6])
)
...
mean_comp_3$names <- colnames(count_matrix)
mean_comp_4$names <- colnames(count_matrix)
mean_comp_5$names <- colnames(count_matrix)
mean_comp_6$names <- colnames(count_matrix)
mean_comp_1_splx$names <- colnames(count_matrix)
mean_comp_2_splx$names <- colnames(count_matrix)
mean_comp_3_splx$names <- colnames(count_matrix)
mean_comp_4_splx$names <- colnames(count_matrix)
mean_comp_5_splx$names <- colnames(count_matrix)
mean_comp_6_splx$names <- colnames(count_matrix)
...
plot10 <- ggplot(mean_comp_4_splx) +
    geom_bar(aes(x = names, y = pc1_plus, color = "Factor 0.15"), 
             stat = "identity", fill = NA) +
    # geom_bar(aes(x = names, y = pc1_minus, color = "Factor -0.09"), 
    #          stat = "identity", fill = NA) +
...
plot11 <- ggplot(mean_comp_5_splx) +
    geom_bar(aes(x = names, y = pc1_plus, color = "Factor 0.12"), 
             stat = "identity", fill = NA) +
    # geom_bar(aes(x = names, y = pc1_minus, color = "Factor -0.09"), 
    #          stat = "identity", fill = NA) +
...
plot12 <- ggplot(mean_comp_6_splx) +
    geom_bar(aes(x = names, y = pc1_plus, color = "Factor 0.09"), 
             stat = "identity", fill = NA) +
    # geom_bar(aes(x = names, y = pc1_minus, color = "Factor -0.09"), 
    #          stat = "identity", fill = NA) +
...
grid.arrange(plot1,plot2,plot3,plot7,plot8,plot9, plot4,plot5,plot6, plot10, plot11,plot12, ncol = 3)
# factor represents the standard deviation of each component

```

```{r insolation data, eval=TRUE, fig.width=12, fig.height=7}
data <- read.table("./Spatio-Temporal-Variations/Linear Analysis/data all/Insolation.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
plot1 <- ggplot(data_long, aes(x = age, y = value, color = variable)) +
  geom_line(size = 1) +  # Add lines for each variable
  labs(x = "age",
       y = "Value",
       title = "Values over age") +
  theme_minimal() +
  # scale_color_manual(values = c("ins_chb_s" = "blue", "ins_chb_a" = "red", "ins_tibet_s" = "green")) +
  ylim(min(data_long$value), max(data_long$value)) # Set y-axis limits to the range of the data

print(plot1)
```

```{r scores robust, eval=TRUE, warning=FALSE, fig.width=12, fig.height=12}
# TODO: make a package function
calculate_pc_scores <- function(data, pca, pc_number) {
    # Center the data
    centered_data <- scale(data, center = TRUE, scale = FALSE)
...
# Fit GAM model
gam_fit1 <- gam(score ~ s(age, k=520, bs="cr"), data=pc1_time_data)
...
# PC2
gam_fit2 <- gam(score ~ s(age, k=520, bs="cr"), data=pc2_time_data)

# Create prediction data
pred_data_2 <- data.frame(age = seq(min(pc2_time_data$age), 
                                 max(pc2_time_data$age), 
                                 length.out=200))
...
plot2 <- ggplot(pc2_time_data, aes(x = age, y = score)) +
    geom_point(alpha=0.5) +
    geom_line(data=pred_data_2, aes(y=fit), color="blue", size=1) +
...
plot3 <- ggplot(pc1_time_data, aes(x = age, y = score)) +
    ...
    geom_ribbon(data=pred_data, aes(y=fit, ymin=lower, ymax=upper), 
                alpha=0.2) +
    geom_line(data = data_long, aes(x = age, y = value, color = "orange"), size = 1) +
    labs(x = "Age (ka)", 
         y = "PC1 Score",
         title = "PC1 Scores over Time") +
    theme_minimal() +
...
plot4 <- ggplot(pc2_time_data, aes(x = age, y = score)) +
    geom_point(alpha=0.5) +
    geom_line(data=pred_data_2, aes(y=fit), color="blue", size=1) +
    geom_ribbon(data=pred_data_2, aes(y=fit, ymin=lower, ymax=upper), 
                alpha=0.2) +
...
```{r block_bootstrap_pca corrected sampling robust, eval=FALSE}
block_bootstrap_pca <- function(data, block_length = 50, R = 1000, reference_pca = NULL) {
  n <- nrow(data)
  results <- list()
  ...
  # Calculate reference PCA if not provided
  if (is.null(reference_pca)) {
    reference_pca <- pcaCoDa(data, scale. = TRUE)
  }
  ...
  for(i in 1:R) {
    ...
    # Perform PCA on bootstrap sample
    boot_data <- data[boot_indices, ]
    boot_pca <- pcaCoDa(boot_data, scale. = FALSE)
    
    # Align signs with reference PCA
    for(j in 1:ncol(boot_pca$loadings)) {
      # Calculate correlation with reference
      cor_sign <- sign(sum(boot_pca$loadings[,j] * reference_pca$loadings[,j]))
      # Flip sign if negative correlation
      if(cor_sign < 0) {
        boot_pca$loadings[,j] <- -boot_pca$loadings[,j]
        boot_pca$x[,j] <- -boot_pca$x[,j]
      }
    }
    
    results[[i]] <- list(
      rotation = boot_pca$loadings,
      sdev = sqrt(boot_pca$eigenvalues)
    )
  }
...
# Anwendung:
bootstrap_results <- block_bootstrap_pca(data, 
                                       block_length = 50, 
                                       R = 1000, 
                                       reference_pca = pca_rob)
