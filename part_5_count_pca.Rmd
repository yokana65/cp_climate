...
plot_pca_rotations(pca_rob$loadings, components = c(1,2), main = "PC1 vs. PC2", fixed=TRUE)
pca_rob$loadings
```
...
```{r principal components rob, eval=TRUE, fig.width=12, fig.height=5}
...
```{r scores robust, eval=TRUE, warning=FALSE, fig.width=12, fig.height=5}
# TODO: make a package function
calculate_pc_scores <- function(data, pca, pc_number) {
    # Center the data
    centered_data <- scale(data, center = TRUE, scale = FALSE)
...
# Fit GAM model
gam_fit1 <- gam(score ~ s(age, k=120, bs="cr"), data=pc1_time_data)
...
# PC2
gam_fit2 <- gam(score ~ s(age, k=120, bs="cr"), data=pc2_time_data)
