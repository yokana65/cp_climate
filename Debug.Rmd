...
conditional_scores_log_ilr_db_2 <- function(scores,
  ...
  # Compute scaled log likelihood
  log_likelihood <- sum(x_data_i * clr_comp)
                                      - sum(x_data_i) * log(norm_constant)
...
monitor_global_ess <- function(all_weights, k) {
  ...
  lambda <- 1
  sum_exp = TRUE
  proposal_scores <- list(length(count_data))
  weights <- list(length(count_data))
  sdev_list <- list(length(max_iter))
  center_list <- list(length(max_iter))
  conditional_scores_list <- list(length(count_data))
  scores_median_list <- list(length(count_data))
  log_weights_list <- list(length(count_data))
  aux_distr_list <- list(length(count_data))
...
scores_median <- do.call(rbind, scores_median_list)
summary(scores_median)
scores_conditional <- do.call(rbind, conditional_scores_list)
summary(scores_conditional)
log_weights <- do.call(rbind, log_weights_list)
summary(log_weights)
aux_distr <- do.call(rbind, aux_distr_list)
summary(aux_distr)
weights
monitor_global_ess(weights, k)
```
...
> pca_ilr$center
[1]  1.4134826 -0.4138567 -2.4568519 -0.5559619
