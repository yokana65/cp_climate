...
gradient_cslc_ilr_db <- function(scores,
  ...
  # scaling_factor <- 0.1
  m_i <- sum(x_data_i)
  ilr_comp <- as.vector(pca_ilr$center + pca_ilr$rotation %*% scores_min)
  clr_comp <- ilr2clr(ilr_comp)
  composition <- clrInv_long(clr_comp)
  ...
  grad_vecs <- sapply(seq_along(scores_min), function(k) {
    ...
    grad_k <- scaling_factor * (v_k * (term1 - term2)) - scores_min[k] / (pca_ilr$sdev[k]^2)
...
testrun1 <- fit_compositional_pca_ilr_sc(x_data, sc_factor = 0.01, sum_exp = TRUE)
testrun1$pca$center
plot(testrun1$pca$sdev)
plot_pca_rotation(testrun1$pca$rotation)

clr_rotation <- t(basis_matrix) %*% testrun1$pca$rotation %*% basis_matrix
pca_clr$rotation
```
