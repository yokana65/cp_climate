...
weswegen es äußerst hilfreich ist, einen statistisch validen Weg zu finden dies auch zu tun.
...
$$.
Dieses generative System kann genutzt werden, um den Kompositionsvektor als Perturbation seiner logarithmierten Kompositionsteile zu beschreiben (Cp. Pawlowsky-Glahn et al 2015, p. 34): 
$$
...
wobei 
$$
\mathrm{g}_{m}(\mathbf{x})=\left(\prod_{j=1}^{D} x_{j}\right)^{1 / D}=\exp \left(\frac{1}{D} \sum_{i=1}^{D} \ln x_{j}\right)
$$
...
entsprechen die Koefficienten in [.. ] den von Aitchison definierten Koefficienten der centered logratio Transformation (clr). Die clr Koordinaten haben eine Reihe von Vorteilen. Insbesondere 
gilt für eine Komposition $\boldsymbol{\pi}_{k} \in \mathcal{S}^{D}$, dass durch die clr-Transformation eine eindeutige Beziehung zwischen den Operationen auf dem Simplex (chapter [...]) und dem 
reelen Raum hergestellt werden kann.
Für realen konstanten $\alpha, \beta$ gilt für alle $k=1,2$:
...
(c) $\left\|\mathbf{x}_{1}\right\|_{a}=\left\|\operatorname{clr}\left(\mathbf{x}_{1}\right)\right\|, \quad \mathrm{d}_{a}\left(\mathbf{x}_{1}, \mathbf{x}_{2}\right)=\mathrm{d}\left(\operatorname{clr}\left(\mathbf{x}_{1}\right), \operatorname{clr}\left(\mathbf{x}_{2}\right)\right)$.
Die Eigenschaft (a) sagt uns, dass die clr-Transformation ein isomorphismus ist, d.h. die Operationen auf dem Simplex werden auf die entsprechenden Operationen im reellen Raum abgebildet. (b) und (c) kennzeichnen 
...
## Orthonormal coordinates or Coordinate Representations (Cp. Filzmoser et al. 2019, p. 43ff)
...
Allerdings lässt sich eine orthonormale Basis des Simplex relativ einfach mit Hilfe der Gram-Schmidt Prozedur konstruieren (Egozcue 2003). 
