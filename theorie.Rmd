...
# calculate clr from ilr coordinates
x_ilr%*%basis_matrix*(-1)
```
...
```{r summary 8, eval=TRUE}
# clr coordinates multiplied by basis_matrix
(x_clr%*%t(basis_matrix))

```

This is equivalent to the negative ilr-coordinates. The sign depends on the direction of our orthonormal basis and has no analytical significance.

Let's check of the log transformation gives us the same results

```{r summary 7, eval=TRUE} 
# log(x) times basis_matrix
(log(x)%*%t(basis_matrix))

```

```{r summary 9, eval=TRUE}
# and back transformation
acomp(exp(x_ilr%*%(basis_matrix*(-1))))
```


## Proves

It must hold that $E*E^T = I_{D-1}$

```{r prove, eval=TRUE}
basis_matrix%*%t(basis_matrix)
```

Thats correct except for the typical numerical error.