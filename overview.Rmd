...
sapply(
        c("rmarkdown", "targets", "visNetwork", "ggplot2", "robCompositions", "compositions", "patchwork"), 
...
To evaluate the point of "sparse sampling" we can look at observations over time. 

```{r hist time, eval=TRUE}
ggplot(data, aes(x = age)) +
  geom_histogram(binwidth = 5, fill = "#5381ca", color = "black") +
  labs(x = "Alter (in tausend Jahren)", y = "Anzahl der Beobachtungen", 
       title = "Verteilung der Beobachtungen über die Zeit (binwidth = 5 Tsd Jahre)") +
  theme_minimal()

```

Comment: First bin is not correct since it starts at 0 and not at 2.017 years.

Observations per time frame (5 Tsd years):

```{r count per time, eval=TRUE}
data_cts <- data %>%
  mutate(age_group = cut(age, breaks = seq(2.017, max(age, na.rm = TRUE), by = 5))) %>%
  group_by(age_group) %>%
  summarise(count = n())
...
# long format would be needed if I want to plot all elements in one plot
# data_long <- pivot_longer(data, cols = all_of(variables), names_to = "Element", values_to = "Count")

# list_p <- list()
# for (var in variables) {
  # hist(data[, var], main = paste("Histogram of", var),
  #      xlab = var, density = TRUE, freq = TRUE)
    # list_p <- ggplot(data, aes(x = var)) +
    # geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") +
    # geom_density(color = "red") +
    # # facet_wrap(~ Element, scales = "free") +
    # theme_minimal() +
    # labs(title = "Histograms of Element ", x = "Count", y = "Density")
# }
# wrap_plots(list_p,ncol = 2)
ggplot(data, aes(x = Br_cts)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") +
    geom_density(color = "red") +
    # facet_wrap(~ Element, scales = "free") +
    theme_minimal() +
...
In general this looks fine except of the Distribution of S.
...
Aitchison compositions: We don't loose any information, when we calculate the composition as parts of a whole:

```{r aitchison composition, eval=TRUE}
data_acomp <- acomp(data_comp)
```

### Exploratory Analysis

We now extend the exploratory methods by the proposed steps from Boogart et al. 2013.
The dataset at hand is the raw element data transformed into an Aitchison composition, i.e. the data is transformed into a simplex with constant sum 1.

```{r aitchison composition test, eval=TRUE}
x <- acomp(data[,3:15])
dim(x)
# error test for a aitchison dataset
row_sums <- rowSums(x)

# Check if all row sums are equal to 1 (within a small tolerance)
all_rows_sum_to_one <- all(abs(row_sums - 1) < 1e-10)

# Print the result
if (all_rows_sum_to_one) {
  cat("All rows in the Aitchison-transformed dataset sum up to 1.\n")
} else {
  cat("Not all rows in the Aitchison-transformed dataset sum up to 1.\n")
}
```
...
Formel: Pawlowski 2015, p. 66, Boogart et al. 2013, p.73
$$
\begin{equation*}
...
Different examples can be used:

```{r Dendrogram example, eval=TRUE}
...
ggplot(clr_coord, aes(x = Ti_cts, y = Al_cts)) +
  geom_point(size = 2) +
  xlab("Ti (clr)") +
  ylab("Al (clr)")
```
...
data_3_comp <- data[, c("Br_cts", "Sr_cts", "Fe_cts")]
