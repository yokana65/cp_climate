...
par(mfrow = c(3,1))
boxplot(data_clr, main="Boxplots of the clr coordinates for each element", las = 2)
# boxplot(data_clr[, variables[variables != "Ca_Area"]], main="Boxplots of the clr coordinates for each element", las = 2)
boxplot(data_ilr, main="Boxplots of the ilr coordinates for each element", las = 2)
boxplot(data_alr, main="Boxplots of the alr coordinates for each element", las = 2)
par(mfrow = c(1,1))
```
...
The eigenvalues are an indication of the explained variance from each component and thus used to order the components. The sum of the eigenvalues is equal to the 
...
```{r, eval=TRUE}
p_comp_rob$princompOutputClr
```


```{r, eval=TRUE}
p_comp_rob$mult_comp
```
...
```{r, eval=TRUE}
(pca_loading <- p_comp_rob$loadings[, 1:2] )
...
# remember that we need the transformed variables in the Simplex. To get the variables in Euclidean space (i.e. the original count space we need to retransform the result)
obs1 <- data[1, variables]
# data[1, variables] %*% pca_loading[, 1]
```
